            ...
            <div className="min-h-[80vh] flex justify-center items-center">
                <div className="w-[95%] lg:w-[40%] mt-5 p-4 mb-10 rounded-xl bg-white bg-dark  mx-auto">
                    <div className="flex items-center gap-4 border-b-2">
                        {tabs.map((tab, idx) => (
                            <button className={`font-bold text-sm border-b-2 pb-2 px-3 ${select === idx ? "text-green-500 border-green-500" : "text-neutral-800 dark:text-white border-transparent"}`} key={idx}>{tab}</button>
                        ))}
                    </div>
