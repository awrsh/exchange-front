      ...
      <Table header={["تاریخ", "تعداد(تراکنش)", "پاداش(تومان)"]} >
        {data.map((item, idx) => (
          <StyledTableRow className="style-table-row" key={idx}>
            ...
            <StyledTableCell className="!font-num dark:!text-white" align="center">{item.count}</StyledTableCell>
            <StyledTableCell className="!font-num dark:!text-white" align="center">{item.reward.toLocaleString()} تومان</StyledTableCell>
          </StyledTableRow>
        ))}
