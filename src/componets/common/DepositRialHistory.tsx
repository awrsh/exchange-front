...
const DepositRialHistory = () => {
    const { isLoading, data, isSuccess } = useDepositRialPaymentHistory()
    // @ts-ignore
                ...
                <Table isLoaidng={isLoading} length={data?.object?.length} header={["ردیف", "نوع", "تاریخ‌وزمان", "مبلغ(تومان)", "شناسه تراکنش", "کارت پرداختی", "وضعیت"]}  >
                    {
                        filteredData?.map((crypto:any, idx:number) => (
                            <StyledTableRow key={idx}>
                                ...
                                <StyledTableCell align="center" className="!font-num dark:!text-white">{Number(crypto.amount/10).toLocaleString()} <span className="text-[10px]">تومان</span></StyledTableCell>
